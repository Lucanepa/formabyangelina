...
import { calLink, calcom } from '@/config/booking'
 ...
 */
export function BookButton({ slug, children, ...props }: BookButtonProps) {
  return (
    <Button
      data-cal-link={calLink(slug)}
      data-cal-config={JSON.stringify({ layout: calcom.layout })}
      {...props}
    >
      {children}
    </Button>
